...
class AdminViewInstances extends Component {
    ...
    // retrieve user document and put reference into this state
    componentWillMount = async () => {
        const { user, _instances } = this.state;
        const userDocument = await firestore.doc(`users/${this.props.id}`).get();
        this.setState({
            _instances: userDocument.data()
        })
    };

    // change instance state and update the datastore with update function
    instanceStart = async (id, name, instanceID, instanceState, instanceType, publicDNS, publicIP, user) => {
        ...
        // update collection
        let eventDocRef = firestore.collection('users').doc(`${this.props.id}`);
        ...
        // change instance state with new data
        const userDocument = await firestore.doc(`users/${this.props.id}`).get();
        this.setState({
            _instances: userDocument.data()
        })
...
export default AdminViewInstances;